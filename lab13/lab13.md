...
#### Решение
1. считаем производную: $N \cdot (1-p)^{N - 1} - N \cdot p \cdot (N - 1)\cdot(1 - p)^{N - 2} = N \cdot (1 - p)^{N - 2} \cdot (1 - p - (N-1)p) = N \cdot (1 - p)^{N - 2} \cdot (1 - Np)$. Максимум достигается, если это выражение равно 0, то есть при $p = 1/N$ ($p \neq 1$, так как тогда сама эффективность была бы равна нулю)
...
#### Решение
1. $p'(1-p')^4$, где $p' = p(1-p)^3$, т.е. A удалось передать информацию. Получаем  $p(1-p)^3 \cdot (1-p(1-p)^3)^4$
